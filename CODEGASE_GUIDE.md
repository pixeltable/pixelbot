...
| `images.py` | `/api` | `GET /generation_config`, `POST /generate_image`, `GET /image_history`, `DELETE /delete_image/{ts}`, `POST /generate_video`, `GET /video_history`, `GET /serve_video`, `DELETE /delete_video/{ts}`, `POST /save_generated_image`, `POST /save_generated_video` |
...
| `studio.py` | `/api/studio` | `GET /files`, `GET /operations`, `POST /search`, `GET /embeddings?space=text\|visual`, `GET /image_preview/{uuid}`, `POST /transform/image`, `POST /save/image`, `POST /download/image`, `POST /transform/video` (metadata, extract_frame, clip_video, overlay_text, detect_scenes), `POST /save/video`, `POST /save/extracted_frame`, `GET /summary/{uuid}`, `GET /chunks/{uuid}`, `GET /frames/{uuid}`, `GET /transcription/{uuid}/{type}`, `POST /csv/rows`, `POST /csv/rows/add`, `PUT /csv/rows/update`, `DELETE /csv/rows/delete`, `POST /csv/revert` (single undo via `table.revert()`), `DELETE /csv/{uuid}` |
| `database.py` | `/api/db` | `GET /tables` (list all tables/views with schema + row counts), `GET /table/{path}/rows` (paginated row fetch), `GET /table/{path}/schema` (detailed column info), `GET /timeline` (unified chronological feed across all timestamped tables) |
...
| `src/App.tsx` | Routes (BrowserRouter): `/`, `/studio`, `/architecture`, `/history`, `/images`, `/memory`, `/database`, `/settings` |
...
| `HistoryPage` | `history/history-page.tsx` | Searchable history list, detail dialog, export JSON. **Debug Export** button downloads the full `agents.tools` table (all 21 columns including computed LLM responses, tool outputs, context retrievals, and follow-ups) for row-level pipeline debugging. |
| `ImagesPage` | `images/images-page.tsx` | Tabbed media generation (Images / Videos). Inline prompt bar for generation via configurable provider (Gemini Imagen / DALL-E for images, Gemini Veo for video). Gallery grid with search, detail dialog, download/delete. Provider badge from `GET /generation_config`. **Save to Collection**: generated images/videos can be saved to the main `agents.images`/`agents.videos` tables via dedicated buttons, triggering CLIP embedding, keyframe extraction, transcription, and RAG indexing automatically. |
...
| `DatabasePage` | `database/database-page.tsx` | Pixeltable catalog browser with two views. **Tables view**: left panel lists all tables/views with icons (green for tables, blue for views), column count, and row count. Clicking a table shows schema header (column chips color-coded: gray for insertable, amber for computed) and a scrollable data grid with pagination. **Timeline view**: unified chronological feed across all timestamped tables (queries, chat, memory, documents, media, generation tasks, CSV imports, personas) with color-coded event type badges, role indicators, and date grouping. Toggled via Tables/Timeline switcher in header. |
...
- **Table revert**: `tbl.revert()` undoes the last operation (insert, update, delete) on a table. Used in the CSV workspace Undo button.
