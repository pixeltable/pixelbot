...
| `functions.py` | `@pxt.udf` and `@pxt.query` functions (news, finance, context assembly, `extract_document_text` for auto-summarization) |
...
| `history.py` | `/api` | `GET /conversations` (list all conversations grouped by conversation_id), `GET /conversations/{id}` (load conversation messages), `DELETE /conversations/{id}` (delete conversation), `GET /workflow_detail/{ts}`, `DELETE /delete_history/{ts}`, `GET /download_history`, `GET /debug_export` (full `agents.tools` table with all 21 columns for debugging) |
...
| `database.py` | `/api/db` | `GET /tables` (list all tables/views with schema + row counts), `GET /table/{path}/rows` (paginated row fetch), `GET /table/{path}/schema` (detailed column info), `GET /timeline` (unified chronological feed across all timestamped tables), `POST /join` (cross-table join with inner/left/cross modes), `GET /pipeline` (full DAG metadata: tables, views, computed column lineage, dependency graph, embedding indices, version history, per-column error counts) |
...
|---|---|---|
| `AppLayout` | `app-layout.tsx` | Sidebar nav (collapsible, 200px/56px) with **grouped navigation** (Conversation: Chat/History/Memory, Workspace: Media Library/Studio/Prompt Lab, System: Database/Developer/Architecture) separated by thin dividers and uppercase group labels. **New chat button** (pencil icon) in header area, visible in both expanded and collapsed states. **Recent conversations** list below Conversation group (up to 8, scrollable 180px max) showing truncated title, relative timestamp, delete on hover. Active conversation highlighted. Settings at bottom. Footer links to Pixeltable GitHub with SVG icon. |
| `ChatPage` | `chat/chat-page.tsx` | **Personalized greeting** ("Good morning/afternoon/evening, {name}") with time-of-day awareness via `/api/user_info`, brand-yellow accent. **Action chips with icons**: mode-aware chips ("Create image", "Create video", "Generate speech") that activate the corresponding mode with color accents, plus neutral chips ("Search the web", "Summarize my files", "Ask anything"). **Multi-conversation support**: each conversation tracked by `conversation_id` (UUID) in URL param `?c=`, persisted to `agents.chat_history`. New chat generates fresh UUID; clicking sidebar conversation loads messages via `/api/conversations/{id}`. Integrated input bar with persona selector + **Image mode**, **Video mode**, and **Voice mode** toggles in bottom toolbar. Chat is the sole creation hub: Image mode generates via Gemini Imagen, Video mode generates via Gemini Veo, Voice mode generates speech via OpenAI TTS — all inline. Proper markdown rendering via `marked` (bold, lists, code blocks, tables, blockquotes). Extracted `MessageBubble` component with memoized HTML; video messages render an inline `<video>` player, audio messages render an inline `<audio>` player. **Thinking summary**: "Thought for Xs" timestamp shown above each assistant response. **Source citation pills**: compact color-coded pill badges (Documents=blue, Images=yellow, Tools=orange, Memory=emerald) with tinted backgrounds. Follow-ups rendered as rounded chip buttons with arrow icons. Copy/save actions on hover. Deps: `marked`. |
| `StudioPage` | `studio/studio-page.tsx` | File explorer + data wrangler. **Left sidebar** (fixed 288px): integrated drag-drop upload zone + URL input at top, vertical file-type filter list with labels + count badges (Documents, Images, Videos, Audio, Tables); auto-selects first non-empty type on load. Separated "Explore" section with cross-modal semantic search input and Embedding Map toggle. File names wrap up to 2 lines via `line-clamp-2`, double-click to rename (persisted in localStorage). **Workspace**: browse files, apply PIL transforms on images, view document chunks (with auto-generated summaries from Gemini), video keyframes, audio/video transcriptions. **Video operations**: metadata inspection (`get_metadata`/`get_duration`), frame extraction (`extract_frame`), clip creation (`clip`), text overlay (`overlay_text`), and scene detection (`scene_detect_content`) — all via Pixeltable's native video UDFs. Save transformed images/videos to Pixeltable or download. Interactive 2-D UMAP embedding map with Text/Visual space toggle, type filtering, click-to-select detail panel with nearest neighbors. **CSV CRUD**: inline cell editing (click-to-edit), add row form, row deletion, **infinite Undo** (repeated `table.revert()` until v0), **Version History** panel (collapsible, shows all versions with change stats via `table.get_versions()`) — all using Pixeltable `insert()`/`update()`/`delete()` primitives with automatic type coercion and registry `row_count` sync. **AI Analysis**: on-demand object detection (DETR) and image classification (ViT) with SVG bounding box overlay, confidence labels, and interactive hover — available on images and individual video frames. |
...
| `DatabasePage` | `database/database-page.tsx` | Pixeltable catalog browser with **Tables** / **Pipeline** view toggle. **Tables view**: Left panel lists all tables/views grouped by category, search filter, Join toggle. Clicking a table shows schema chips, row filter, Download CSV, scrollable data grid with pagination. **Cross-Table Join panel**: pick two tables, match columns, join type (INNER/LEFT/CROSS). **Pipeline view** (`pipeline-inspector.tsx`): live React Flow DAG from `get_metadata()` / `get_versions()` / `errortype`. **Hierarchical tree layout** — base tables at top, views flow downward via labeled edges showing iterator type (`FrameIterator`, `AudioSplitter`, `DocumentSplitter`, `StringSplitter`). **Dashed cross-table query edges** show `@pxt.query` RAG retrieval flow (e.g., `chunks` → `tools` via `search_documents`), color-coded per source table. **Function classification**: each computed column tagged as `built-in` (sky, Pixeltable native), `UDF` (violet, custom `@pxt.udf`), or `query` (teal, `@pxt.query`). Domain-aware node headers with gradient tints. Stats bar + legend with function type indicators. Click-to-select detail panel: lineage, embedding indices, insertable columns with inheritance tags, computed pipeline with expandable `computed_with` + function name/type + dependency tags, version history, per-column error counts. |
| `DeveloperPage` | `developer/developer-page.tsx` | **Developer** — data export, API reference, SDK snippets, and integration hub. **Export tab**: table picker (auto-lists all Pixeltable tables with column/row info), format selector (JSON/CSV/Parquet), row limit, 5-row live preview, download button + curl equivalent. **API tab**: categorized endpoint reference (Chat & Agent, Files, Generation, Prompt Lab, Data Export, Database, Memory) with method badges (color-coded GET/POST/PUT/DELETE), expandable curl examples with copy. **Python SDK tab**: curated code snippets (connect, query, semantic search, export to Pandas, image similarity, insert + computed columns, version control). **Connect tab**: MCP setup (JSON config for Claude/Cursor), Direct Python access, REST API curl, **Data Sharing** (`pxt.publish()` / `pxt.replicate()` / `t.push()` / `t.pull()` with code examples for publishing to and replicating from Pixeltable Cloud), and external links (Pixeltable docs, GitHub, MCP server, Data Sharing docs, Public Datasets, LLMs.txt). |
...
- **Table schema introspection**: Use the public `tbl.get_metadata()` API — returns a dict with `columns` (each has `type_`, `computed_with`, `is_stored`, etc.), `is_view`, `base` (parent table path for views), `indices`, `version`. Do NOT use `tbl.column_types()` (Pixeltable's `__getattr__` intercepts it as a column name).
- **Catalog browsing**: `pxt.list_tables(namespace, recursive=True)` lists all tables/views. `tbl.columns()` returns column names. `tbl.count()` returns row count. `tbl.get_metadata()['is_view']` and `tbl.get_metadata()['base']` replace `tbl.get_base_table()` for view detection.
...
- **Transient error retry** (`utils.py`): `@pxt_retry(max_attempts=3, delay=0.5, backoff=2.0)` decorator wraps all Pixeltable-facing endpoint functions. Retries on bare `AssertionError` (from internal `assert not Env.get().in_xact` guards), `INTRANS`, `This Connection is closed`, `assert self._current_conn is not None`, and `not initialized` errors. Supports both sync and async functions. Applied across **all routers** (chat, files, studio, images, database, experiments, export, history, memory).
