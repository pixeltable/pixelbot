...
| `DetectionPanel` | `studio/detection-panel.tsx` | Reusable component for on-demand ML analysis. Model selector dropdown (DETR ResNet-50/101 for detection, ViT Base for classification), confidence threshold slider, SVG bounding box overlay with color-coded labels (8-color palette), interactive hover (highlight box + label chip), classification bar chart for ViT results. Used in both `ImageWorkspace` and `VideoWorkspace` (per-frame detection). |
...
| `DatabasePage` | `database/database-page.tsx` | Pixeltable catalog browser. Left panel lists all tables/views with icons (green for tables, blue for views), column count, and row count. Clicking a table shows schema header (column chips color-coded: gray for insertable, amber for computed) and a scrollable data grid with pagination. |
...
- **On-demand ML inference**: `studio.py` uses HuggingFace `transformers` directly (not Pixeltable computed columns) for on-demand object detection (DETR) and image classification (ViT). Models are loaded lazily and cached in `_model_cache` dict for fast subsequent calls. Results include bounding boxes (x1,y1,x2,y2) for detection and top-k class probabilities for classification.
