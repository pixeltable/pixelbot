...
def init_schema(*, force_reset: bool = False) -> None:
    """Create or update the agents schema. Idempotent unless force_reset=True."""
    if force_reset:
    ...
    video_transcript_sentences_view.add_embedding_index(
        column="text", string_embed=gemini_embed, idx_name="video_sentences_gemini", if_exists="ignore"
    )
    ...
    def search_video_transcripts(query_text: str, user_id: str = config.DEFAULT_USER_ID):
        ...
        return (
            video_transcript_sentences_view.where((video_transcript_sentences_view.user_id == user_id) & (sim > 0.7))
            .order_by(sim, asc=False)
    ...
    audio_transcript_sentences_view.add_embedding_index(
        column="text", string_embed=gemini_embed, idx_name="audio_sentences_gemini", if_exists="ignore"
    )
    ...
    def search_audio_transcripts(query_text: str, user_id: str = config.DEFAULT_USER_ID):
        ...
        return (
            audio_transcript_sentences_view.where((audio_transcript_sentences_view.user_id == user_id) & (sim > 0.6))
            .order_by(sim, asc=False)
    ...
    memory_bank.add_embedding_index(
        column="content", string_embed=gemini_embed, idx_name="memory_gemini", if_exists="ignore"
    )
    ...
    chat_history.add_embedding_index(
        column="content", string_embed=gemini_embed, idx_name="chat_history_gemini", if_exists="ignore"
    )
    ...
    tool_agent.add_computed_column(tool_output=invoke_tools(tools, tool_agent.initial_response), if_exists="ignore")

    # Step 3: Context retrieval
    tool_agent.add_computed_column(doc_context=search_documents(tool_agent.prompt, tool_agent.user_id), if_exists="ignore")
    tool_agent.add_computed_column(image_context=search_images(tool_agent.prompt, tool_agent.user_id), if_exists="ignore")
