...
@pxt.udf
def query_csv_table(query: str) -> str:
    """Search uploaded CSV/spreadsheet data using key-based lookups.

    Use this when the user asks about data in their uploaded CSV files.
    The query should be a JSON string with optional fields:
      - "lookup": {"column_name": "value"} for precise key-based row lookup
      - "table": "table_display_name" to target a specific table
      - "question": "natural language question" for general queries

    Examples:
      '{"lookup": {"category": "electronics"}, "table": "products.csv"}'
      '{"question": "what are the top products by price?"}'
      '{"lookup": {"sku": "LAPTOP-001"}}'
    If the query is not valid JSON, it is treated as a natural language question.
    """
    ...
    try:
        ...
        tables_info = []
        for row in registry.select(
            registry.table_name, registry.display_name,
            registry.col_names, registry.row_count,
        ).collect():
            tables_info.append({
        ...
        if not tables_info:
            return "No CSV tables have been uploaded yet."

        # Parse query: JSON with lookup params, or plain text
        lookup_params: dict = {}
        ...
        try:
            ...
            if isinstance(parsed, dict):
                lookup_params = parsed.get("lookup", {})
                target_table = parsed.get("table")
                question = parsed.get("question", query)
        except (_json.JSONDecodeError, TypeError):
            pass

        # If we have lookup params, use retrieval_udf for precise key-based lookup
        if lookup_params:
            for info in tables_info:
                if target_table and target_table.lower() not in info["display_name"].lower():
                    continue
                valid_cols = [k for k in lookup_params if k in info["columns"]]
                if not valid_cols:
                    continue
                try:
                    tbl = pxt.get_table(info["table_name"])
                    udf = pxt.retrieval_udf(
                        tbl,
                        name=f"lookup_{info['table_name'].split('.')[-1]}",
                        description=f"Look up rows in {info['display_name']}",
                        parameters=valid_cols,
                        limit=20,
                    )
                    params = {c: lookup_params[c] for c in valid_cols}
                    result = tbl.select(udf(**params)).limit(1).collect()
                    if result:
                        udf_key = f"lookup_{info['table_name'].split('.')[-1]}"
                        rows = result[0].get(udf_key, [])
                        if rows:
                            return (
                                f"Lookup results from {info['display_name']} "
                                f"(matched {len(rows)} rows on {valid_cols}):\n"
                                + _json.dumps(rows[:20], indent=2, default=str)
                            )
                except Exception as lookup_err:
                    # Fall through to sample-based approach
                    pass

        # Fallback: list tables with sample rows for the LLM to reason over
        parts = [f"Available CSV tables ({len(tables_info)}):"]
        for info in tables_info:
            parts.append(f"\n{info['display_name']} ({info['row_count']} rows)")
            parts.append(f"  Columns: {', '.join(info['columns'])}")
            try:
                tbl = pxt.get_table(info["table_name"])
                sample = tbl.select().limit(5).collect()
                if sample:
                    parts.append("  Sample rows:")
                    for i, r in enumerate(sample[:3]):
                        row_str = ", ".join(
                            f"{c}: {r.get(c)}" for c in info["columns"] if r.get(c) is not None
                        )
        ...
        parts.append("Answer based on the data above.")
        return "\n".join(parts)
