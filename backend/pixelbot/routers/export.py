...
from pixelbot.catalog_access import registered_scratch_tables, require_allowed_table
from pixelbot.models import ExportTablesResponse, JsonColumnResponse, PreviewResponse
...
def _collect_rows(table_path: str, limit: int, columns: list[str] | None) -> tuple[list[str], list[dict]]:
    ...
    table_path = require_allowed_table(table_path)
    try:
...
def export_json_column(
    ...
    try:
        tbl = pxt.get_table(require_allowed_table(table_path))
    except Exception:
        raise HTTPException(status_code=404, detail=f"Table not found: {table_path}")
...
# ── Native Pixeltable 0.7.7 export (pxt.io.export_csv / export_json) ───────────
