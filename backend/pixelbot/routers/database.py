"""Read-only Pixeltable catalog and lineage inspection API."""
...
def sample_table(body: SampleRequest):
    ...
    """
    try:
        tbl = pxt.get_table(require_allowed_table(body.path))
    except Exception:
...
def get_timeline(limit: int = 100):
    ...
    TIMELINE_SOURCES = [
        ("pixelbot_v3/tools", "prompt", "Query"),
        ("pixelbot_v3/chat_history", "content", "Chat"),
        ("pixelbot_v3/memory_bank", "content", "Memory"),
        ("pixelbot_v3/collection", "document", "Document"),
        ("pixelbot_v3/images", "image", "Image"),
        ("pixelbot_v3/videos", "video", "Video"),
        ("pixelbot_v3/audios", "audio", "Audio"),
        ("pixelbot_v3/image_generation_tasks", "prompt", "ImageGen"),
        ("pixelbot_v3/video_generation_tasks", "prompt", "VideoGen"),
        ("pixelbot_v3/speech_tasks", "input_text", "Speech"),
        ("pixelbot_v3/csv_registry", "display_name", "CSV"),
        ("pixelbot_v3/user_personas", "persona_name", "Persona"),
    ]
...
def join_tables(body: JoinRequest):
    ...
    try:
        left = pxt.get_table(require_allowed_table(body.left_table))
    except Exception:
        raise HTTPException(status_code=404, detail=f"Table not found: {body.left_table}")
    try:
        right = pxt.get_table(require_allowed_table(body.right_table))
    except Exception:
...
# @pxt.query from pixelbot/schema.py — maps query name to the table it searches
