...
from pixelbot.models import ImageRow, SaveImageToCollectionResponse, VideoRow
    ...
    "image": [
        {
            ...
            "category": "transform",
            "params": [
                {"name": "width", "type": "number", "default": 256, "min": 16, "max": 4096},
                {"name": "height", "type": "number", "default": 256, "min": 16, "max": 4096},
            ],
        },
        {
            "id": "rotate",
    ...
    "document": [
        {
            "id": "view_chunks",
            "label": "View Chunks",
            "description": "See extracted text chunks from the document",
            "category": "analyze",
            "params": [],
        },
    ],
    "audio": [
        {
            ...
            "description": "See audio transcription",
            "category": "analyze",
            "params": [],
        },
    ],
    "video": [
        {
            "id": "view_metadata",
            "label": "Metadata",
            "description": "Show resolution, duration, fps, codec info",
            "category": "analyze",
            "params": [],
        },
        {
            "id": "view_frames",
            "label": "View Keyframes",
            "description": "See extracted video keyframes",
            "category": "analyze",
        ...
        },
        {
            "id": "view_transcription",
            "label": "View Transcription",
            "description": "See video audio transcription",
            "category": "analyze",
            "params": [],
        },
        {
            "id": "detect_scenes",
            "label": "Detect Scenes",
            "description": "Find scene cuts with timestamps",
            "category": "analyze",
        ...
        },
        {
            "id": "extract_frame",
            "label": "Extract Frame",
            "description": "Extract a single frame at a timestamp",
            "category": "transform",
        ...
        },
        {
            "id": "clip_video",
            "label": "Clip Video",
            "description": "Extract a portion of the video",
            "category": "transform",
            "params": [
                {"name": "start", "type": "time", "default": 0.0, "min": 0.0},
        ...
        },
        {
            "id": "crop_video",
            "label": "Crop Video",
            "description": "Crop a rectangular region (bbox) of the video",
            "category": "transform",
            "params": [
                {"name": "x", "type": "number", "default": 0, "min": 0},
        ...
        },
        {
            "id": "concat_videos",
            "label": "Concat Videos",
            "description": "Concatenate multiple videos into one (same resolution required)",
            "category": "transform",
            "params": [
                {
                    "name": "uuids",
                    "type": "string",
                    "default": "",
                    "description": "Comma-separated UUIDs of videos to concatenate",
                }
            ],
        },
    ],
...
def get_studio_files():
    ...
    try:
        ...
        for row in (
            ...
            .order_by(doc_table.timestamp, asc=False)
            .collect()
        ):
            doc_entry: dict = {
                "uuid": row["uuid_col"],
    ...
    try:
        ...
        for row in (
            ...
            .select(
                ...
                ts=img_table.timestamp,
                thumb=img_table.thumbnail,
            )
            .order_by(img_table.timestamp, asc=False)
            .collect()
        ):
            thumbnail = None
    ...
    try:
        ...
        for row in (
            ...
            .select(
                ...
                ts=vid_table.timestamp,
            )
            .order_by(vid_table.timestamp, asc=False)
            .collect()
        ):
    ...
    try:
        ...
        for row in (
            ...
            .select(
                ...
                ts=audio_table.timestamp,
            )
            .order_by(audio_table.timestamp, asc=False)
            .collect()
        ):
...
def _get_csv_tables(user_id: str) -> list[dict]:
    ...
    try:
        ...
        tables: list[dict] = []
        for row in (
            registry.where(registry.user_id == user_id)
            .select(
                registry.table_name,
            ...
            .order_by(registry.timestamp, asc=False)
            .collect()
        ):
            tables.append(
                {
...
def _verify_csv_ownership(csv_uuid: str, user_id: str):
    """Resolve a client-facing registry UUID to an owned scratch table."""
    registry = pxt.get_table("pixelbot_v3.csv_registry")
    check = (
        registry.where((registry.uuid == csv_uuid) & (registry.user_id == user_id))
        .select(registry.table_name, registry.col_names, registry.row_count, registry.uuid)
        .collect()
    )
    if not check:
        raise HTTPException(status_code=404, detail="CSV table not found")
    table_name = check[0]["table_name"]
    allowed_prefixes = (f"{config.SCRATCH_NAMESPACE}.", f"{config.SCRATCH_NAMESPACE}/")
    if not table_name.startswith(allowed_prefixes):
        logger.error("Rejected invalid scratch registry path for UUID %s", csv_uuid)
        raise HTTPException(status_code=500, detail="Invalid CSV registration")
    return check[0]
...
def search_studio(body: SearchRequest):
    ...
    if "document" in body.types:
        try:
            ...
            for row in (
                chunks_view.where((chunks_view.user_id == user_id) & (sim > body.threshold))
                .select(
    ...
    if "image" in body.types:
        try:
            ...
            for row in (
                img_table.where((img_table.user_id == user_id) & (sim > body.threshold))
    ...
    if "video" in body.types:
        try:
            ...
            for row in (
                frames_view.where((frames_view.user_id == user_id) & (sim > body.threshold))
                .select(
...
def get_embeddings(space: str = "text", limit: int = Query(default=200, ge=2, le=500)):
    ...
    if not valid_mask.all():
        ...
        valid_flags = cast(list[bool], np.asarray(valid_mask).tolist())
        items = [item for item, ok in zip(items, valid_flags) if ok]
        if len(matrix) < 2:
            return {"space": space, "points": items, "count": len(items)}
...
def _collect_text_embeddings(
    ...
    try:
        chunks_view = pxt.get_table("pixelbot_v3.chunks")
        for row in (
            chunks_view.where(chunks_view.user_id == user_id)
            .select(
                text=chunks_view.text,
                uuid_col=chunks_view.uuid,
...
def _collect_visual_embeddings(
    ...
    try:
        frames_view = pxt.get_table("pixelbot_v3.video_frames")
        for row in (
            frames_view.where(frames_view.user_id == user_id)
            .select(
                uuid_col=frames_view.uuid,
                frame=frames_view.frame,
                frame_attrs=frames_view.frame_attrs,
...
def list_detection_models():
    ...
    return [{"key": key, "type": info["type"], "label": info["label"]} for key, info in DETECTION_MODELS.items()]
