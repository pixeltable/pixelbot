...
from pixelbot.utils import create_thumbnail_base64, encode_image_base64, pxt_retry, resolve_served_media_path
...
def generate_slideshow(body: GenerateSlideshowRequest):
    ...
    imagen_table = pxt.get_table("pixelbot_v3.image_generation_tasks")
    flux_table = pxt.get_table("pixelbot_v3.flux_generation_tasks")
    ...
    for ts_str in body.timestamps:
        ...
        # Try Imagen
        res = (
            imagen_table.where((imagen_table.timestamp == target_ts) & (imagen_table.user_id == user_id))
            .select(img=imagen_table.generated_image)
            .collect()
        )
        ...
        # Try Flux
        res = (
            flux_table.where((flux_table.timestamp == target_ts) & (flux_table.user_id == user_id))
            .select(img=flux_table.generated_image)
            .collect()
        )
        if res and res[0].get("img"):
            images.append(res[0]["img"])
