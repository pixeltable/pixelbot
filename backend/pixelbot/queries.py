...
#
# These mirror the @pxt.query functions in pixelbot/schema.py but are designed
...
#
# The @pxt.query versions in pixelbot/schema.py remain for computed columns.
...
def get_all_memory(user_id: str) -> list[dict]:
    ...
    t = pxt.get_table("pixelbot_v3.memory_bank")
    return list(
...
def get_all_personas(user_id: str) -> list[dict]:
    ...
    t = pxt.get_table("pixelbot_v3.user_personas")
    return list(
        t.where(t.user_id == user_id)
        .select(
