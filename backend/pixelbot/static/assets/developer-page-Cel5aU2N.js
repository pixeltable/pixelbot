...
print(status.rows[0]["answer"])`},{label:"FastAPIRouter v2 API",description:"Declarative table-backed routes (memory, personas)",language:"python",code:`# Pixelbot registers FastAPIRouter routes at startup.
...
import requests
rows = requests.get("http://localhost:8000/api/memory/v2").json()["rows"]`},{label:"Native Export (0.6+)",description:"Export tables with pxt.io.export_csv / export_json",language:"python",code:`import pixeltable as pxt
