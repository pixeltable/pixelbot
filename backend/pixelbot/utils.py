...
def _is_transient(exc: Exception) -> bool:
    ...
    try:
        ...
        if isinstance(exc, PxtError):
            ...
            return any(message in err_str for message in _TRANSIENT_MESSAGES)
    except ImportError:
        pass
    return False
...
def allowed_media_roots() -> list[Path]:
    ...
    if pixeltable_home := os.environ.get("PIXELTABLE_HOME"):
        roots.append(Path(pixeltable_home).expanduser().resolve())
