...
const FUNC_STYLES: Record<string, { text: string; label: string }> = {
  builtin: { text: 'text-muted-foreground/70', label: 'built-in' },
  custom_udf: { text: 'text-k-yellow/80', label: 'UDF' },
  query: { text: 'text-blue-400/80', label: 'query' },
  unknown: { text: 'text-muted-foreground/50', label: 'fn' },
}
      ...
      {label && (
        <EdgeLabelRenderer>
          <div
            ...
            className={cn(
              'absolute pointer-events-all px-1.5 py-0.5 rounded border text-[8px] font-mono',
              isQuery
                ? 'bg-card border-border/40 text-muted-foreground/60'
...
function TableNode({ data }: { data: PipelineNode & { isSelected: boolean; onSelect: (path: string) => void; hasIncoming: boolean; hasOutgoing: boolean } }) {
      ...
      <div className="px-3 py-2 border-b border-border/40">
        <div className="flex items-center gap-1.5">
          {data.is_view ? (
            ...
            <Table2 className="h-3 w-3 text-muted-foreground/60 shrink-0" />
          )}
          <span className="text-[11px] font-semibold text-foreground truncate">{data.name}</span>
        </div>
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          ...
          {data.iterator_type && (
            <span className="text-[8px] px-1 rounded bg-accent text-muted-foreground font-mono">
              {data.iterator_type}
            </span>
          )}
        </div>
      ...
      <div className="px-2.5 py-1.5 space-y-px">
        {insertableCols.slice(0, 3).map((col) => (
          <div key={col.name} className="flex items-center gap-1.5">
            ...
            <span className="text-[9px] text-muted-foreground/40 truncate">{col.name}</span>
          </div>
        ))}
        {insertableCols.length > 3 && (
          <div className="text-[8px] text-muted-foreground/25 pl-2.5">+{insertableCols.length - 3} more</div>
        )}
        ...
        {computedCols.length > 0 && (
          <>
            ...
            {computedCols.slice(0, 4).map((col) => {
              ...
              return (
                <div key={col.name} className="flex items-center gap-1">
                  ...
                  <span className={cn(
                    'text-[9px] truncate',
                    col.error_count > 0 ? 'text-destructive/80' : 'text-foreground/70',
                  )}>
                    {col.name}
                  </span>
                  {col.func_name && ft && (
                    <span className={cn('text-[7px] shrink-0 font-mono', ft.text)}>
                      {col.func_name}
                    </span>
                  )}
                </div>
            ...
            {computedCols.length > 4 && (
              <div className="text-[8px] text-muted-foreground/25 pl-2.5">+{computedCols.length - 4} more</div>
            )}
          </>
        )}
      </div>
    ...
    <div className="w-[360px] shrink-0 border-l border-border bg-card overflow-y-auto">
      ...
      {node.base && (
        <div className="px-5 py-3 border-b border-border flex items-center gap-2">
          ...
          <span className="text-xs text-foreground font-mono font-medium">{node.base}</span>
        </div>
      )}
            ...
            {node.indices.map((idx) => (
              <div key={idx.name} className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground break-words leading-relaxed mt-0.5">{idx.embedding}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {insertable.length > 0 && (
        <div className="px-5 py-3 border-b border-border">
          ...
          <div className="space-y-1">
            {insertable.map((col) => (
              <div key={col.name} className="flex items-baseline gap-2 text-xs min-h-[20px]">
                <div className="w-1.5 h-1.5 rounded-full bg-muted-foreground/20 shrink-0 mt-[5px]" />
                <span className="text-foreground/80 shrink-0">{col.name}</span>
                {col.defined_in && !col.defined_in_self && (
                  <span className="text-[10px] text-muted-foreground/50 italic shrink-0">from {col.defined_in}</span>
                )}
                <span className="text-muted-foreground/40 ml-auto text-[11px] font-mono text-right shrink-0">{col.type}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="space-y-1 ml-1">
              {node.versions.map((v) => (
                <div key={v.version} className="flex items-center gap-3 text-xs">
                  ...
                  <span className={cn(
                    'px-1.5 py-0.5 rounded text-[10px] min-w-[50px] text-center',
                    v.change_type === 'schema' ? 'bg-accent text-muted-foreground' : 'bg-accent/50 text-muted-foreground/70',
                  )}>
                    {v.change_type}
                  </span>
...
function SectionLabel({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={cn('text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-2', className)}>
      {children}
    </div>
  )
}
...
function ComputedColumnRow({ col, step }: { col: PipelineColumn; step: number }) {
  ...
  return (
    <div className="rounded-md border border-border overflow-hidden">
      ...
      >
        <span className="text-[10px] text-muted-foreground w-5 shrink-0 tabular-nums">#{step}</span>
        <span className={cn(
        ...
        {ft && (
          <span className={cn('text-[10px] px-1.5 py-0.5 rounded bg-accent font-mono shrink-0', ft.text)}>
            {ft.label}
          </span>
        )}
      ...
      {isExpanded && (
        <div className="px-3 pb-3 pt-2 space-y-2 border-t border-border">
          {col.func_name && (
            <div className="flex items-center gap-2">
              <span className={cn('text-xs font-mono font-medium', ft?.text ?? 'text-muted-foreground')}>
                {col.func_name}()
              </span>
              {ft && (
                <span className="text-[10px] text-muted-foreground">
                  {col.func_type === 'builtin' ? 'pixeltable built-in' : col.func_type === 'custom_udf' ? '@pxt.udf' : col.func_type === 'query' ? '@pxt.query' : ''}
                </span>
              )}
            </div>
          )}
          {col.computed_with && (
            <pre className="text-[11px] text-muted-foreground font-mono leading-relaxed bg-accent rounded-md px-3 py-2 whitespace-pre-wrap break-all">
              {col.computed_with}
            </pre>
          )}
          {col.depends_on && col.depends_on.length > 0 && (
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {col.depends_on.map((d) => (
                <span key={d} className="text-[10px] bg-accent text-muted-foreground px-1.5 py-0.5 rounded">{d}</span>
              ))}
            </div>
          )}
          {col.error_count > 0 && (
