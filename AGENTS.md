...
The entire tool-calling agent is a chain of `add_computed_column()` calls. Inserting a row triggers the full pipeline: tool planning → execution → multimodal RAG → context assembly → final answer → follow-up generation. The router just inserts and reads back.
