...
| `images.py` | `POST /generate_image`, `POST /generate_flux_image`, `GET /image_history`, `GET /flux_image_history`, `POST /generate_video`, `GET /video_history`, `POST /generate_speech`, `POST /save_generated_image`, `POST /save_flux_image`, `POST /save_generated_video` |
| `memory.py` | `POST /memory`, `POST /memory/manual`, `GET /memory`, `DELETE /memory/{ts}` |
| `personas.py` | CRUD on `/personas` |
| `studio.py` | `/api/studio/` — file browsing, image/video transforms (crop bbox, concat), embeddings, CSV CRUD with undo, AI detection, Reve AI editing |
| `database.py` | `/api/db/` — table listing, paginated rows, schema, sampling, timeline, cross-table join, pipeline DAG, **pipeline management** (create/drop/rename tables, add/drop/rename columns, computed columns, views, embedding indexes, directory CRUD, type & function discovery) |
...
| `/images` | `ImagesPage` | Media Library — Imagen + FLUX + Veo gallery, Reve AI editing |
...
| `/database` | `DatabasePage` | Pixeltable catalog browser, pipeline DAG inspector, sampling, joins, **full table/column/view/index CRUD** via management toolbar |
...
8. **Image generation** — Gemini Imagen + BFL FLUX
...
All LLM calls use `gemini.generate_content()` and `gemini.embed_content()` (multimodal embeddings for text, image, video, audio). Only exceptions: Whisper (transcription), TTS (speech), CLIP (visual cross-modal search), BFL FLUX (image generation/editing). Claude/Mistral/OpenAI models are still available in the Prompt Lab for multi-model comparison, but the agent pipeline is all-Gemini. Message format uses `role: "user"/"model"` with `parts: [{text: "..."}]`.
...
- **Reve AI**: `pixeltable.functions.reve` — `create(prompt)`, `edit(image, edit_instruction)`, `remix(prompt, images)`. Requires `REVE_API_KEY`.
- **BFL FLUX**: `pixeltable.functions.bfl` — `generate(prompt, model)`, `edit(prompt, image, model)`, `expand(...)`, `fill(...)`. Requires `BFL_API_KEY`.
- **Video UDFs (v0.5.21)**: `video.crop(video, bbox, bbox_format)`, `video.concat_videos_agg(order_col, video_col)`, `video.segment_video(...)`, `video.with_audio(...)`.
...
## Database Management API (Pipeline Builder)

Full CRUD for Pixeltable objects via `database.py` (`/api/db/`). All management endpoints return `MgmtResponse` with `{success, message, path, detail}`.

| Endpoint | Method | Purpose |
|---|---|---|
| `/api/db/create_dir` | POST | Create namespace directory |
| `/api/db/drop_dir` | POST | Drop directory (optionally force) |
| `/api/db/create_table` | POST | Create table with `{path, schema: {col: type_string}}` |
| `/api/db/drop_table` | POST | Drop table/view |
| `/api/db/rename_table` | POST | Rename/move table |
| `/api/db/revert_table` | POST | Undo last operation |
| `/api/db/insert_rows` | POST | Insert rows `{path, rows: [...]}` |
| `/api/db/delete_rows` | POST | Delete rows matching equality filter |
| `/api/db/add_column` | POST | Add plain column `{path, column_name, column_type}` |
| `/api/db/add_computed_column` | POST | Add computed column with expression string |
| `/api/db/drop_column` | POST | Remove column |
| `/api/db/rename_column` | POST | Rename column |
| `/api/db/create_view` | POST | Create view with optional iterator |
| `/api/db/add_embedding_index` | POST | Add embedding index ("gemini" or "clip") |
| `/api/db/drop_embedding_index` | POST | Remove embedding index |
| `/api/db/table/{path}/versions` | GET | Version history for a table |
| `/api/db/types` | GET | Available column types |
| `/api/db/functions` | GET | Available functions, iterators, embedding functions |

**Type strings** for schemas: `string`, `int`, `float`, `bool`, `timestamp`, `json`, `image`, `video`, `audio`, `document`, `Array[<type>]`.

**Expression strings** for computed columns use a namespace with `table.col_name` references and function modules (`gemini`, `openai`, `image`, `video`, `string`).

**Iterators** for views: `DocumentSplitter`, `FrameIterator`, `AudioSplitter`, `StringSplitter` — column-reference args are resolved from the base table.
